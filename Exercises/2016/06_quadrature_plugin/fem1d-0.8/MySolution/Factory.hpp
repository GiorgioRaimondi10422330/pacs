  ...
  */
  ...
  class Factory{
    ...
    */
    ...
    //! The builder type.    /*       The default is a function. In C++98 I would have used      a function pointer. Here I prefer to use a function wrapper instead.      I return a unique_ptr (C++11). This can make the handling more complex      but it is much safer with respect to memory leaks. A simpler version      may use a bare pointer as return type.    */
    using  Builder_type=Builder;
    //! Method to access the only instance of the factory
    static Factory & Instance();
    //! Get the rule with given name .      /*!       The pointer is null if no rule is present. @todo use variadic templates to make it more general       */
  ...
  template<typename Identifier> std::string identifierAsString(Identifier const & id)
   {
    return
   ...
   */
  template<typename AbstractProduct,typename Identifier,typename Builder >
  std::unique_ptr<AbstractProduct> Factory<AbstractProduct,Identifier,Builder>::create(Identifier const & name) const {
  ...
  template<typename AbstractProduct,typename Identifier,typename Builder>
  void  Factory<AbstractProduct,Identifier,Builder>::add(Identifier const & name, Builder_type const & func){
    auto f = 
  ...
  template<typename AbstractProduct,typename Identifier,typename Builder >
  std::vector<Identifier>  Factory<AbstractProduct,Identifier,Builder>::registered() const {
