...
template<class prec>
  ...
  )
  {
    ...
    std::vector<std::vector<prec> > A=a;
    std::vector<prec> C=c;
    K_compute(K,f,N,NK,y0,t0,A,C,h);
...
template<class prec>
     ...
     )
  {
    ...
    cout<<"errorPerTimeStep="<<errorPerTimeStep<<endl;
        ...
        unsigned int count=0;
      	...
      	while (h> h_min && localError > c1*errorPerTimeStep)
            ...
            count++;
      	    ...
      	    errorPerTimeStep /=2;
      	    newy = rk_step(dy,y,time,h,localError);
            cout<<"Al passo "<<stepsCounter<<" dimezzo h per la "<<count<<"a volta     h="<<h<<"  err="<<localError<<"/"<<errorPerTimeStep*3.<<"\n";
      	  }
	      if (localError>errorPerTimeStep)status=1;
      	//! advance
      	y = newy;
        //if(stepsCounter<20) cout<<"localError= "<<localError<<endl;
      	time +=h;
        if(stepsCounter<20) cout<<stepsCounter<<"   Tempo= "<<time<<endl;
      	++stepsCounter;
